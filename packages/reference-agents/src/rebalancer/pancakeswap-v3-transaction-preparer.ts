...
/**
 * Computes the token mix required by the target range at the observed pool
 * price using exact raw-unit arithmetic. No USD float is used for sizing.
 */
export function planRebalanceComposition(input: {
  ...
  if (available0Raw > desired0RawBeforeSwap) {
    return {
      ...
      available0Raw,
      available1Raw,
      desired0RawBeforeSwap,
      desired1RawBeforeSwap,
      swapDirection: "TOKEN0_TO_TOKEN1",
      swapAmountInRaw: available0Raw - desired0RawBeforeSwap
    };
  }
  if (available1Raw > desired1RawBeforeSwap) {
    return {
      ...
      swapAmountInRaw: available1Raw - desired1RawBeforeSwap
    };
  }
  return {
    targetTickLower: target.tickLower,
...
function approvalCalls(input: {
  ...
  input.calls.push({
    order: input.calls.length,
    kind: "approval-reset",
    label: `Reset ${input.label} allowance`,
    to: input.token,
    data: encodeFunctionData({ abi: ERC20_APPROVAL_ABI, functionName: "approve", args: [input.spender, 0n] }),
    value: "0",
    asset: input.token,
    amount: "0",
  ...
  input.calls.push({
    order: input.calls.length,
    kind: "approval",
    label: `Set exact ${input.label} allowance`,
    to: input.token,
    data: encodeFunctionData({ abi: ERC20_APPROVAL_ABI, functionName: "approve", args: [input.spender, input.amount] }),
    value: "0",
    asset: input.token,
...
function revokeCall(input: {
  ...
  input.calls.push({
    ...
    label: `Revoke ${input.label} allowance`,
    to: input.token,
    data: encodeFunctionData({ abi: ERC20_APPROVAL_ABI, functionName: "approve", args: [input.spender, 0n] }),
    value: "0",
    asset: input.token,
...
function executionCommitment(input: {
  proposalId: string;
...
export class PancakeV3UnsignedTransactionPreparer {
  ...
  async prepare(input: {
    ...
    const composition = planRebalanceComposition({
    ...
    calls.push({
      order: calls.length,
      kind: "protocol",
      label: `Remove all liquidity from Pancake V3 position ${snapshot.tokenId}`,
      to: npm,
      data: encodeFunctionData({
        abi: NPM_ABI,
        functionName: "decreaseLiquidity",
        args: [{
          tokenId: BigInt(snapshot.tokenId),
          liquidity: snapshot.positionLiquidity,
    ...
    calls.push({
      order: calls.length,
      kind: "protocol",
      label: `Collect principal and fees from position ${snapshot.tokenId}`,
      to: npm,
      data: encodeFunctionData({
        abi: NPM_ABI,
    ...
    if (composition.swapDirection !== "NONE" && composition.swapAmountInRaw > 0n) {
      ...
      const tokenInDecimals = composition.swapDirection === "TOKEN0_TO_TOKEN1"
        ? snapshot.token0Decimals
        : snapshot.token1Decimals;
      const tokenOutDecimals = composition.swapDirection === "TOKEN0_TO_TOKEN1"
        ? snapshot.token1Decimals
        : snapshot.token0Decimals;

      swapQuote = await this.swapQuoteProvider.quoteExactInput({
        tokenIn,
        tokenOut,
        tokenInDecimals,
        tokenOutDecimals,
        amountIn: composition.swapAmountInRaw,
        recipient: owner,
      ...
      if (getAddress(swapQuote.tokenIn) !== tokenIn || getAddress(swapQuote.tokenOut) !== tokenOut) {
        throw new Error("SWAP_QUOTE_ASSET_MISMATCH");
      }
      ...
      approvalCalls({
        ...
        token: tokenIn,
        spender: router,
        amount: composition.swapAmountInRaw,
        label: "Pancake swap"
      });
      calls.push({
        order: calls.length,
        kind: "swap",
        label: `Swap exact ${composition.swapAmountInRaw.toString()} raw units for target LP composition`,
        to: router,
        data: swapQuote.calldata,
        value: swapQuote.value.toString(),
        asset: tokenIn,
        amount: composition.swapAmountInRaw.toString(),
        spender: router
      });
      revokeCall({ calls, token: tokenIn, spender: router, label: "Pancake swap" });

      if (composition.swapDirection === "TOKEN0_TO_TOKEN1") {
        expectedPostSwap0 -= composition.swapAmountInRaw;
        expectedPostSwap1 += swapQuote.expectedAmountOut;
      } else {
        expectedPostSwap1 -= composition.swapAmountInRaw;
        expectedPostSwap0 += swapQuote.expectedAmountOut;
      }
    }
    ...
    calls.push({
      order: calls.length,
      kind: "protocol",
      label: `Mint replacement Pancake V3 range ${composition.targetTickLower}:${composition.targetTickUpper}`,
      to: npm,
      data: encodeFunctionData({
        abi: NPM_ABI,
        functionName: "mint",
        args: [{
          ...
          token1,
          fee: snapshot.fee,
          tickLower: composition.targetTickLower,
          tickUpper: composition.targetTickUpper,
          amount0Desired: expectedPostSwap0,
          amount1Desired: expectedPostSwap1,
    ...
    return {
      ...
      spendBounds: [
        ...(swapQuote ? [{
          asset: swapQuote.tokenIn,
          maxAmount: swapQuote.amountIn.toString(),
          spender: swapQuote.router,
          purpose: "Exact-input composition swap"
        }] : []),
        { asset: token0, maxAmount: expectedPostSwap0.toString(), spender: npm, purpose: "Replacement V3 mint token0" },
        { asset: token1, maxAmount: expectedPostSwap1.toString(), spender: npm, purpose: "Replacement V3 mint token1" }
      ],
      evidenceRefs: [
        ...input.prepared.evidenceRefs,
        ...(swapQuote?.evidenceRefs ?? []),
        ...(swapQuote ? [swapQuote.routeRef] : [])
      ],
      limitations: [
        ...
        "Any material state or quote drift requires invalidating this action and preparing a new commitment."
      ]
    };
  }
}
