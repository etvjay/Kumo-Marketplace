...
import { preparedActionSchema } from "../packages/financial-agent-kernel/dist/index.js";
import {
  PANCAKESWAP_V3_BSC,
...
const snapshot = {
  ...
  tickLower: -64979,
  tickUpper: 887272,
  positionLiquidity: 3882124761380135480n,
  feeGrowthInside0LastX128: 0n,
  feeGrowthInside1LastX128: 0n,
  tokensOwed0: 1000000000000000n,
  tokensOwed1: 0n,
  pool: POOL,
  sqrtPriceX96: 3016472752945240429896451150n,
  currentTick: -65369,
  poolLiquidity: 119600319973320857700709n,
  unlocked: true,
  blockNumber: 118704833n,
  blockHash: "0xdf92b38e525fae003fe19fc4d6f417ae799e25814110a5b2bdfac6d198a71363",
  blockTimestamp: 1787978273,
  blockTag: "finalized",
  purpose: "evidence",
  snapshot: {
    chainId: 56,
...
const prepared = {
  ...
  token1Price: {},
  valuation: {
    principalAmount0Raw: 100000000000000000000n,
    principalAmount1Raw: 0n,
    principalAmount0: 100,
    principalAmount1: 0,
    principalValueUsd: 100,
    crystallizedFeesFloorUsd: 0,
    markedValueIncludingCrystallizedFeesUsd: 100
  },
  baseline: {},
  evidenceRefs: [
    "fixture:bsc:block:118704833",
    "fixture:pancakeswap-v3:position:999001"
  ]
};

const proposal = {
  id: "proposal:fixture:rebalancer:999001",
  agentId: "kumo-rebalancer-reference-bsc-v1",
  category: "rebalancing",
  mode: "shadow",
  createdAt: "2026-08-29T04:59:59.000Z",
  expiresAt: "2026-08-29T05:00:20.000Z",
  objective: "TEST FIXTURE: exercise a complete unsigned V3 rebalance preparation",
  action: "TEST FIXTURE: recenter position 999001",
  disposition: "propose",
  rationale: "TEST_FIXTURE_ONLY — not derived from a live strategy verdict",
  expectedNetBenefit: 5,
  estimatedCost: 1,
  evidencePacketRef: "fixture:evidence",
  evidenceSnapshotRoot: "fixture:evidence-root:v1",
  marketSnapshotRoot: "fixture:market-root:v1",
  refusalReasons: []
};
...
const fixtureQuoteProvider = {
  ...
  async quoteExactInput(input) {
    quoteCalls += 1;
    const expectedAmountOut = input.tokenIn.toLowerCase() === USDT.toLowerCase()
      ? input.amountIn / 690n
      : input.amountIn * 689n;
    return {
      ...
      value: 0n,
      gasEstimate: 150000n,
      routeRef: "fixture:pancake-v3:quote",
      evidenceRefs: ["fixture:quote:evidence"]
    };
  }
};

const output = {
  schemaVersion: "kumo-rebalancer-prepared-action-fixture-v1",
...
try {
  ...
  const exactCommitmentReproducible = first.executionCommitment === second.executionCommitment;
  ...
  const commitmentShape = /^0x[0-9a-fA-F]{64}$/.test(first.executionCommitment);

  const passed = ordersContiguous
    && exactCommitmentReproducible
    && hasRemove
    && hasCollect
    && hasSwap
    && hasMint
  ...
  output.composition = {
    targetTickLower: composition.targetTickLower,
    targetTickUpper: composition.targetTickUpper,
    swapDirection: composition.swapDirection,
    swapAmountInRaw: composition.swapAmountInRaw
  };
  ...
  output.status = passed ? "PREPARED_ACTION_FIXTURE_PASS" : "PREPARED_ACTION_FIXTURE_FAIL";
  if (!passed) process.exitCode = 1;
} catch (error) {
  output.status = "PREPARED_ACTION_FIXTURE_ERROR";
